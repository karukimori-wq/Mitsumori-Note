...
export type InvoiceLine={id:number;name:string;qty:number;price:number};
export type BillingRecord={projectId:string;lines:InvoiceLine[];issued:boolean;paid:boolean;dueDate:string;paidDate:string;updatedAt:string};
export type ReviewRecord={projectId:string;score:number;note:string;updatedAt:string};
const KEYS={projects:"mitsumori-note-projects",customers:"mitsumori-note-customers",estimates:"mitsumori-note-estimates",actuals:"mitsumori-note-actuals",billings:"mitsumori-note-billings",reviews:"mitsumori-note-reviews"};
...
export function loadBillings(){if(typeof window==="undefined")return[] as BillingRecord[];try{return JSON.parse(localStorage.getItem(KEYS.billings)||"[]") as BillingRecord[]}catch{return[]}}
export function getBilling(projectId:string){return loadBillings().find(x=>x.projectId===projectId)}
export function saveBilling(projectId:string,data:Omit<BillingRecord,"projectId"|"updatedAt">){const all=loadBillings().filter(x=>x.projectId!==projectId);const record:BillingRecord={projectId,...data,updatedAt:new Date().toISOString()};localStorage.setItem(KEYS.billings,JSON.stringify([record,...all]));if(data.paid)updateProject(projectId,{status:"入金済み"});else if(data.issued)updateProject(projectId,{status:"請求済み"});return record}
export function loadReviews(){if(typeof window==="undefined")return[] as ReviewRecord[];try{return JSON.parse(localStorage.getItem(KEYS.reviews)||"[]") as ReviewRecord[]}catch{return[]}}
export function getReview(projectId:string){return loadReviews().find(x=>x.projectId===projectId)}
export function saveReview(projectId:string,score:number,note:string){const all=loadReviews().filter(x=>x.projectId!==projectId);const record:ReviewRecord={projectId,score,note,updatedAt:new Date().toISOString()};localStorage.setItem(KEYS.reviews,JSON.stringify([record,...all]));return record}
